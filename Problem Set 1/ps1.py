...
def greedy_cow_transport(cows, limit=10):
    ...
    while len(sorted_cows) > 0:
        ...
        for c in sorted_cows[:]:
            if cows[c] <= remaining_limit:
                subresult.append(c)
                remaining_limit -= cows[c]
...
def compare_cow_transport_algorithms(cows):
    ...
    result = round(end - start, 10)
    print("Brute force algorithm: {0}".format(result))
    
    start = time.time()
    greedy_cow_transport(cows)
    end = time.time()
    result = end - start
    print("     Greedy algorithm: {:.10f}".format(float(result)))
